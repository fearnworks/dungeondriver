                ...
                with tqdm.tqdm(total=total_size, unit='iB', unit_scale=True, bar_format='{l_bar}{bar}| {n_fmt:6}/{total_fmt:6} {rate_fmt:6}') as t:
                    ...
                    for data in r.iter_content(block_size):
                        ...
                        if total_size != 0 and progress_bar is not None:
                            count += len(data)
                            progress_bar(float(count) / float(total_size), f"Downloading {filename}")
    ...
    while True:
        ...
        for i in range(len(dict)):
            ...
            if fname in ["1_Pooling","2_Dense"]:
                ...
                links.append(f"https://huggingface.co/{model}/resolve/{branch}/{fname}/config.json")
                links.append(f"https://huggingface.co/{model}/resolve/{branch}/{fname}/pytorch_model.bin")
                continue
...
if __name__ == '__main__':
    ...
    for model in models:
        ...
        session.mount('https://cdn-lfs.huggingface.co', HTTPAdapter(max_retries=retries))
        session.mount('https://huggingface.co', HTTPAdapter(max_retries=retries))
        session.headers = {'authorization': f'Bearer {os.getenv("HUGGINGFACE_API_TOKEN")}'}
        ...
        thread_map(lambda url: download_file(url, output_folder), links, max_workers=threads, disable=True)
